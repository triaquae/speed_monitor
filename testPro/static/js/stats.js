...
(function() {
	...
	if(w.YAHOO && w.YAHOO.widget && w.YAHOO.widget.Logger) {
		...
		//boomr.log = function(m,l,s) { console.log(s + ": [" + l + "] " + m); }; comment out by alex
		boomr.log = function(m,l,s) { var log = null };
	} 
...
impl = {
	...
	initNavTiming: function() {
		...
		if(ti) {
			...
			//coment out by Alex BOOMR.addVar("rt.start", source || "navigation");
...
BOOMR.plugins.RT = {
	...
	// load when the page is usable by the user
		...
		if(ename==="xhr" && edata.name && impl.timers[edata.name]) {
			...
			//commented out by alex!   BOOMR.addVar("rt.start", "manual");
		}
			...
			//count out by alex BOOMR.addVar("rt.start", "cookie");	// if the user hit the back button, referrer will match, and cookie will match
		}						// but will have time of previous page start, so t_done will be wrong
		else {
			//count out by alex  BOOMR.addVar("rt.start", "none");
		...
		if(ntimers) {
			if(ename !== "xhr") {
				//count out by alex BOOMR.addVar("r", BOOMR.utils.cleanupURL(impl.r));
	...
	complete: false,
		...
		if(p && p.timing && p.navigation) {
			...
			data = {
				...
				dom_ready: pt.domComplete - pt.navigationStart,
				//on_load : pt.loadEventEnd - pt.navigationStart ,
				response_time : pt.responseEnd - pt.requestStart ,
				first_byte : pt.responseEnd -  pt.navigationStart
				//domContent_ready :  pt.domContentLoadedEventEnd - pt.domContentLoadedEventStart ,
				
				
			};
			if (pt.secureConnectionStart) {
				// secureConnectionStart is OPTIONAL in the spec
				var nt_ssl_st = pt.secureConnectionStart;
				//data.nt_ssl_st = pt.secureConnectionStart;
			}
			if (pt.msFirstPaint) {
				// msFirstPaint is IE9+ http://msdn.microsoft.com/en-us/library/ff974719
				var nt_first_paint = pt.msFirstPaint;
				data.nt_first_paint = pt.msFirstPaint;
				//added by alex
				data.nt_first_paint_time = pt.msFirstPaint - pt.navigationStart;
			}
		...
		if(w.chrome && w.chrome.loadTimes) {
			...
			if(pt) {
				data = {
					...
					nt_first_paint: pt.firstPaintTime *1000,
					//added by alex
					nt_first_paint_time : pt.firstPaintTime *1000 - p.timing.navigationStart,
					chrome_navi : p.timing.navigationStart
